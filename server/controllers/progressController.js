...
import curriculum from "../data/curriculum.json" with { type: "json" }; //assert eski süürm
...
export const getProgress = async (req, res) => {
  try {
    ...
    // Map'i düz objeye çevir (JSON için)
    const progressObj = {};
    if (progressDoc.progress instanceof Map) {
      for (const [subjectId, topics] of progressDoc.progress) {
        progressObj[subjectId] = {};
        if (topics instanceof Map) {
          for (const [topicId, completed] of topics) {
            progressObj[subjectId][topicId] = completed;
          }
        }
      }
    } else {
      // Zaten obje formatındaysa direkt kullan
      Object.assign(progressObj, progressDoc.progress);
    }

    res.status(200).json({
      progress: progressObj,
      totalCompleted: progressDoc.totalCompleted || 0,
    });
  } catch (error) {
...
// Tek Bir Konuyu Tamamla/Geri Al
export const toggleTopic = async (req, res) => {
    console.log("İşlem yapan kullanıcı ID:", req.user.id);
  try {
    ...
    if (!subjectId || !topicId) {
      return res
        .status(400)
        .json({ message: "Ders ID ve Konu ID gereklidir!" });
    }
    ...
    // Progress objesini düzenle
    const progressData =
      progressDoc.progress instanceof Map
        ? Object.fromEntries(progressDoc.progress)
    ...
    // Toggle işlemi
    progressData[subjectId][topicId] = !progressData[subjectId][topicId];
    ...
    res.status(200).json({
      ...
      completed: progressData[subjectId][topicId],
    });
  } catch (error) {
...
export const getCurriculum = async (req, res) => {
  try {
    // curriculum.json dosyasından direkt döndür
...
export const getProgressStats = async (req, res) => {
  try {
    ...
    console.log("Bulunan Doküman:", progressDoc); // Eğer null ise 1. maddeye bak.
    if (!progressDoc) {
    ...
    curriculum.tyt.forEach((subject) => {
      ...
      if (progressDoc.progress[subject.id]) {
        const completed = subject.topics.filter(
          (topic) => progressDoc.progress[subject.id][topic.id]
        ).length;
        completedTopics += completed;
        tytCompleted += completed;
      }
    });
...
/*
    ...
    // Map yapısı için .get() kullanıyoruz
    const subjectProgress = progressDoc.progress.get(subject.id.toString());
    
    if (subjectProgress) {
        const completed = subject.topics.filter(
        (topic) => subjectProgress.get(topic.id.toString()) === true
        ).length;
